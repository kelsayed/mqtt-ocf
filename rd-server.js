...
var result,
	...
	processCallCount = 0,
	processLoop = null,
	iotivity = require( "iotivity-node/lowlevel" ), //  require( "iotivity-node/lowlevel" )
	testUtils = require( "../tests/utils" )( iotivity );//this loads tests/util.js not sure what (iotivity) parameter does
/*
...
processLoop = setInterval( function() {
	...
	//console.log(JSON.stringify( { processCallCount: processCallCount } ) );
...
result = iotivity.OCCreateResource( terminationResourceHandleReceptacle,
	...
	function terminationResourceEntityHandler( flag, request ) {
		if ( request.resource === terminationResourceHandleReceptacle.handle &&
			...
			//cleanup();
			console.log(JSON.stringify( { CleanMeUp: true } ) );
		}
